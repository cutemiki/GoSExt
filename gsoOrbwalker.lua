...
-- update
	local Files =
	{
		...
		{
			Path = SCRIPT_PATH,
			Name = "gsoOrbwalker.lua",
			Url = "https://raw.githubusercontent.com/gamsteron/GoSExt/master/gsoOrbwalker.lua"
		},
		Version =
		{
			Path = SCRIPT_PATH,
			Name = "gsoOrbwalker.version",
			Url = "https://raw.githubusercontent.com/gamsteron/GoSExt/master/gsoOrbwalker.version"
		}
	}
...
local Linq, BuffManager, ItemManager, Utilities, Damage, ObjectManager, TargetSelector, HealthPrediction, Orbwalker, HoldPositionButton
-- locals
	...
	META1 =
	{
		...
		CURSOR = function()
			...
			function c:CastKey()
				...
				if self.Work ~= nil and Utilities:GetDistance2DSquared(newpos, _G.cursorPos) <= MAXIMUM_MOUSE_DISTANCE then
					self.Work()
					self.Work = nil
				end
			end
		...
		FARM = function()
			...
			function c:GetLastHitTarget()
				...
				for i = 1, #self.FarmMinions do
					...
					if minion.LastHitable and minion.PredictedHP < min and Utilities:IsValidTarget(minion.Minion) and Utilities:IsInAutoAttackRange(myHero, minion.Minion) then
			...
			function c:GetLaneClearTarget()
				...
				if LocalGameTimer() > self.ShouldWaitTime + MENU_CHAMP.lclear.swait:Value() * 0.001 then
					...
					for i = 1, #self.FarmMinions do
						...
						if target.PredictedHP < min and Utilities:IsValidTarget(target.Minion) and Utilities:IsInAutoAttackRange(myHero, target.Minion) then
		...
		ORB = function()
			...
			function c:GetTarget()
				...
				elseif Orbwalker.Modes[ORBWALKER_MODE_LANECLEAR] then
					if FARM.IsLastHitable then
						result = FARM:GetLastHitTarget()
					elseif not FARM.ShouldWait then
...
-- replicate control
	...
	_G.Control.CastSpell = function(key, a, b, c)
		if CONTROLL == nil and LocalGameTimer() > NEXT_CONTROLL + 0.05 then
			...
			if spell == _Q then
				if LocalGameTimer() < SPELLS.LastQ + 0.25 then
					return false
				else
					SPELLS.LastQ = LocalGameTimer()
				end
			elseif spell == _W then
				if LocalGameTimer() < SPELLS.LastW + 0.25 then
					return false
				else
					SPELLS.LastW = LocalGameTimer()
				end
			elseif spell == _E then
				if LocalGameTimer() < SPELLS.LastE + 0.25 then
					return false
				else
					SPELLS.LastE = LocalGameTimer()
				end
			elseif spell == _R then
				if LocalGameTimer() < SPELLS.LastR + 0.25 then
					return false
				else
